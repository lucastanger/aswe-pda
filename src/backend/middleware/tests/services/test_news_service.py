...
def service(mocker, parameters):
    mocker.patch('src.backend.middleware.src.services.news_service.NewsService.get_db_info', paper_id=PAPER_ID, category=CATEGORY)
...
class TestNewsService:
    ...
    def test_top_news_success(self, mocked_get, service):
        ...
        mocked_get.assert_called_with(f'{BASE_URL}/news/{self.__type_top}/category?category={CATEGORY}')
